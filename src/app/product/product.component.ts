...
export class ProductComponent {
  ...
  render() {
    ...
    const html = this.listSP.map(item=>{
      return `<div class="col-md-3 mt-3">
      <div class="card">
        <img class="card-img-top" src="${item.urlImage}" alt="Card image cap">
        <div class="card-body">
          <h5 class="card-title">${item.tensp} ${item.code}</h5>
          <p class="card-text">${item.mota}</p>
          <p class="card-text">Giá: ${item.giasp.toLocaleString('vi-VN', {style: 'currency', currency: 'VND'})} VND</p>
        </div>
      </div>
    </div>
    `
